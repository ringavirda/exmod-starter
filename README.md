...
A starter monorepo for [Expanded Library](https://github.com/ringavirda/modding-vsexlib) mods, generated by `exmod starter` from exlib 0.8.1. It carries one mod per sample exlib's own manifest names, in the family layout (`mods/<id>/{src,assets,tests}`):
